...
export const mergeChatMetasWithUserBios = async (chatMetas: ChatMetaEntity[]) => {
	const userIds = [...new Set(chatMetas.flatMap((cm) => cm.members))]
	...
	return chatMetas.map((cm) => cm.withUsers(Object.fromEntries([...cm.members].map((id) => [id, usersMap.get(id) ?? null]))))
}
